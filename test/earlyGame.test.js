...
// Tests for src/web/earlyGame.js (early-game.html) -- a standalone
...
const html = renderEarlyGame();

test('early-game.html has exactly one <h1> and no heading level is skipped', () => {
...
test('early-game.html carries the canonical link, JSON-LD, and a title/description within the site\'s length caps', () => {
  assert.ok(html.includes(`<link rel="canonical" href="${site.absoluteUrl('early-game.html')}">`));
...
test('early-game.html links to tracker.html, downloads.html, and the focus menu', () => {
...
test('early-game.html links to a real drill anchor and a real focus-menu anchor, not a made-up id', () => {
...
test('early-game.html carries no manual ad-slot placeholder', () => {
  assert.equal((html.match(/class="ad-slot"/g) || []).length, 0);
});

test('program.html and focus-menu.html both link to early-game.html', () => {
  const { CONTENT_PAGES } = require('../src/web/contentPages.js');
  const rendered = new Map(CONTENT_PAGES.map(([name, render]) => [name, render()]));
  assert.ok(rendered.get('program.html').includes(site.url('early-game.html')), 'program.html should link to early-game.html');
  assert.ok(rendered.get('focus-menu.html').includes(site.url('early-game.html')), 'focus-menu.html should link to early-game.html');
});
